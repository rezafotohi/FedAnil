...
class ClientsGroup(object):
	...
	def dataSetBalanceAllocation(self):
		femnist_dataset = GetDataSet(self.data_set_name, self.is_iid)

		# perpare training data
		train_data = femnist_dataset.train_data
		train_label = femnist_dataset.train_label
		# shard dataset and distribute among clients
		# shard train
		shard_size_train = femnist_dataset.train_data_size // self.num_of_clients // 2
		shards_id_train = np.random.permutation(femnist_dataset.train_data_size // shard_size_train)

		# perpare test data
		if not self.shard_test_data:
			test_data = torch.tensor(femnist_dataset.test_data)
			test_label = torch.argmax(torch.tensor(femnist_dataset.test_label), dim=1)
			test_data_loader = DataLoader(TensorDataset(test_data, test_label), batch_size=100, shuffle=False)
		else:
			test_data = femnist_dataset.test_data
			test_label = femnist_dataset.test_label
			 # shard test
			shard_size_test = femnist_dataset.test_data_size // self.num_of_clients // 2
			shards_id_test = np.random.permutation(femnist_dataset.test_data_size // shard_size_test)
